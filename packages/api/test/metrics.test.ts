...
describe('Metrics API', () => {
  ...
  describe('Database operations', () => {
    it('should enforce foreign key constraint', async () => {
      // Try to insert metrics for non-existent worker
      await expect(
        env.bb.prepare(
          `INSERT INTO metrics (worker_id, memory, cpu, rate, created_at)
           VALUES (?, ?, ?, ?, ?)`
        ).bind('wrk_nonexistent', 4096, 45, 12, new Date().toISOString()).run()
      ).rejects.toThrow()
    })

    it('should cascade delete metrics when worker is deleted', async () => {
      ...
      const createRes = await app.request('/api/workers', {
        ...
        body: JSON.stringify({
          name: 'Test Worker for Delete'
        })
      }, env)

      const createData = await createRes.json()
      const testWorkerId = createData.worker_id

      // Insert metrics
      await env.bb.prepare(
        `INSERT INTO metrics (worker_id, memory, cpu, rate, created_at)
         VALUES (?, ?, ?, ?, ?)`
      ).bind(testWorkerId, 4096, 45, 12, new Date().toISOString()).run()

      // Delete worker (revoke it, which marks it as revoked but doesn't actually delete)
      // For cascade delete to work, we need to actually DELETE from the database
      await env.bb.prepare(`DELETE FROM workers WHERE id = ?`).bind(testWorkerId).run()

      // Verify metrics are deleted via cascade
      const metrics = await env.bb.prepare(
        `SELECT * FROM metrics WHERE worker_id = ?`
      ).bind(testWorkerId).all()

      expect(metrics.results.length).toBe(0)
    })
  })
})
